...
public record AddEntries(CommonData commonData, List<LootPoolEntryContainer> entries) implements LootModifier {
    ...
    public void apply(LootTable table, ResourceLocation key) {
        List<LootPool> pools = accessor(table).datapatched$getPools();
        for (LootPool pool : pools) {
            var baseEntries = ImmutableList.<LootPoolEntryContainer>builder().addAll(accessor(pool).datapatched$getEntries()).addAll(this.entries).build();

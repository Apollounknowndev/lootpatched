...
public record AddPools(CommonData commonData, List<LootPool> pools) implements LootModifier {
    ...
    public void apply(LootTable table, ResourceLocation key) {
        List<LootPool> fullPools = ImmutableList.<LootPool>builder().addAll(accessor(table).datapatched$getPools()).addAll(this.pools).build();
        accessor(table).datapatched$setPools(fullPools);
    }

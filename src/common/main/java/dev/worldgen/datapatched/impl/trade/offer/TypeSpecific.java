...
public record TypeSpecific(Map<ResourceKey<VillagerType>, Holder<TradeOffer>> trades, Holder<TradeOffer> fallback) implements TradeOffer {
    ...
    public static final MapCodec<TypeSpecific> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
        Codec.unboundedMap(TYPE_CODEC, TradeOffer.ENTRY_CODEC).fieldOf("trades").forGetter(TypeSpecific::trades),
        TradeOffer.ENTRY_CODEC.fieldOf("fallback").orElse(EMPTY).forGetter(TypeSpecific::fallback)
    ).apply(instance, TypeSpecific::new));

    public TypeSpecific(Map<ResourceKey<VillagerType>, Holder<TradeOffer>> trades) {
        this(trades, EMPTY);
    }
    ...
    public List<MerchantOffer> create(AbstractVillager merchant, RandomSource random) {
        if (merchant instanceof VillagerDataHolder container) {
            ...
            return this.trades.getOrDefault(key, this.fallback).value().create(merchant, random);
        }
